     ...
     try{
        ...
        if(role){
            const updatedRole = await Role.update({
                 roleCode, roleName, privileges, description, isActive, unregisteredDate},
                {
                    where: {
                        roleID
                    }
...
export async function inactivateRole(req, res){
    ...
    try{
        ...
        if(role){
            const inactivateRole = await Role.update(
                { isActive, unregisteredDate: sequelize.fn('NOW') },
                {
                    where: {
                        roleID
                    }
...
export async function activateRole(req, res){
    ...
    try{
        ...
        if(role){
            const inactivateRole = await Role.update(
                { isActive, registeredDate: sequelize.fn('NOW') },
                {
                    where: {
                        roleID
                    }
